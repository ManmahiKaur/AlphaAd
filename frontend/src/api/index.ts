...
export const stocksApi = {
  ...
  getHistory: async (ticker: string, period = '1Y') =>
    (await api.get<{ candles: CandlePoint[] }>(`/stocks/${encodeURIComponent(ticker)}/history`, { params: { period } })).data,
  getIndicators: async (ticker: string, period = '1Y') =>
    (await api.get<IndicatorsData>(`/stocks/${encodeURIComponent(ticker)}/indicators`, { params: { period } })).data,
};
...
export const aiApi = {
  getRecommendation: async (ticker: string) =>
    (await api.get<Recommendation>(`/analysis/recommendation/${encodeURIComponent(ticker)}`)).data,
