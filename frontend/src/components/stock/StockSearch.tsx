    ...
    <div ref={dropdownRef} className={`relative w-full max-w-xl ${className}`}>
      <form onSubmit={handleSubmit} className="relative flex items-center">
        ...
        <input
          ...
          className="w-full bg-white border border-slate-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 text-slate-900 placeholder-slate-400 rounded-xl pl-11 pr-24 py-3 text-sm outline-none shadow-xl transition-all"
        />
        {query && (
          <button
            type="button"
        ...
        <button
          type="submit"
          className="absolute right-2 px-4 py-1.5 bg-blue-600 hover:bg-blue-700 text-white font-semibold text-xs rounded-lg transition-colors shadow-md shadow-blue-500/20"
        >
      ...
      {isOpen && (
        <div className="absolute top-full left-0 right-0 mt-2 bg-white/95 backdrop-blur-xl border border-slate-200 rounded-xl shadow-2xl overflow-hidden z-50 divide-y divide-slate-200 max-h-96 overflow-y-auto">
          {/* Active Search Results */}
          {query.trim().length >= 2 && (
            <div>
              <div className="px-4 py-2 text-[11px] font-bold text-slate-700 uppercase tracking-wider bg-blue-50 flex items-center justify-between">
                <span>Matching Equities ({results.length})</span>
                <span className="text-blue-600 text-[10px]">Live Search</span>
              </div>
              {isLoading ? (
                <div className="p-4 text-center text-xs text-slate-400">Searching markets...</div>
              ) : results.length === 0 ? (
                <div className="p-4 text-center text-xs text-slate-400">
                  No equities matching "{query}" found.
                </div>
              ) : (
                results.map((item) => (
                  <button
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span className="px-2 py-1 bg-blue-50 text-blue-600 font-mono text-xs font-bold rounded border border-blue-200 group-hover:bg-blue-100">
                        {item.ticker}
                      </span>
                      <div>
          ...
          {recentSearches.length > 0 && query.trim().length === 0 && (
            <div>
              <div className="px-4 py-2 text-[11px] font-bold text-slate-700 uppercase tracking-wider bg-blue-50 flex items-center gap-1.5">
                <Clock className="w-3.5 h-3.5 text-blue-600" /> Recent Searches
              </div>
              {recentSearches.map((item) => (
                ...
                >
                  <div className="flex items-center gap-2.5">
                    ...
                    <span className="text-xs text-slate-500 truncate max-w-xs">{item.name}</span>
                  </div>
                  <span className="text-[10px] text-slate-400 uppercase">{item.country}</span>
                </button>
              ))}
            </div>
          )}
