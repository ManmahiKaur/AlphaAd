...
export const StockCard: React.FC<{ quote: StockQuote }> = ({ quote }) => {
  ...
  return (
    <Card className="flex flex-col justify-between h-full group hover:border-blue-300 transition-all">
      <div>
        ...
        <h4 className="text-xs text-slate-500 line-clamp-1 font-medium mb-3">{quote.name}</h4>
      ...
      <div className="pt-3 border-t border-slate-200 flex items-center justify-between text-xs text-slate-500">
        <div>
          ...
          <span className="font-semibold text-slate-700">{formatMarketCap(quote.market_cap, quote.currency)}</span>
        </div>
        <Link
          to={`/stock/${quote.ticker}`}
          className="p-1.5 rounded-lg bg-slate-100 text-slate-400 group-hover:bg-blue-600 group-hover:text-white transition-all"
        >
