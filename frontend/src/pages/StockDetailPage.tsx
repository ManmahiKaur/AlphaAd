    ...
    <div className="space-y-8">
      ...
      <div className="glass-card p-6 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-3xl font-extrabold text-slate-900">{quote.ticker}</h1>
            <Badge variant={quote.country === 'IN' ? 'orange' : 'blue' as any}>{quote.exchange}</Badge>
            <span className="text-xs text-slate-500 font-semibold">{quote.sector} • {quote.industry}</span>
          </div>
          <h2 className="text-sm text-slate-600 font-medium mt-1">{quote.name}</h2>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button onClick={handleToggleWatchlist} variant="secondary">
              <Bookmark className={`w-4 h-4 ${inWatchlist ? 'fill-blue-600 text-blue-600' : ''}`} />
            </Button>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3">
        <Card className="p-3.5 text-center">
          ...
          <span className="text-sm font-bold font-mono text-slate-900">{formatMarketCap(quote.market_cap, quote.currency)}</span>
        </Card>
        <Card className="p-3.5 text-center">
          <span className="block text-[10px] text-slate-500 uppercase font-semibold">P/E Ratio</span>
          <span className="text-sm font-bold font-mono text-slate-900">{quote.pe_ratio ? quote.pe_ratio : 'N/A'}</span>
        </Card>
        <Card className="p-3.5 text-center">
          <span className="block text-[10px] text-slate-500 uppercase font-semibold">52W High</span>
          <span className="text-sm font-bold font-mono text-emerald-600">{formatCurrency(quote.high_52w || 0, quote.currency)}</span>
        </Card>
        <Card className="p-3.5 text-center">
          <span className="block text-[10px] text-slate-500 uppercase font-semibold">52W Low</span>
          <span className="text-sm font-bold font-mono text-rose-600">{formatCurrency(quote.low_52w || 0, quote.currency)}</span>
        </Card>
        <Card className="p-3.5 text-center">
          <span className="block text-[10px] text-slate-500 uppercase font-semibold">Day Range</span>
          <span className="text-xs font-bold font-mono text-slate-900">{quote.day_low} - {quote.day_high}</span>
        </Card>
        <Card className="p-3.5 text-center">
          <span className="block text-[10px] text-slate-500 uppercase font-semibold">Volume</span>
          <span className="text-sm font-bold font-mono text-slate-900">{quote.volume.toLocaleString()}</span>
        </Card>
      </div>
      ...
      <Card className="p-6">
        <h3 className="text-base font-bold text-slate-900 mb-4 flex items-center gap-2">
          <Activity className="w-5 h-5 text-blue-600" /> Technical Trading Chart
        </h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-4">
          <h3 className="text-base font-bold text-slate-900">Ask AI Advisor About {quote.ticker}</h3>
