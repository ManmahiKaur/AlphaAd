    ...
    <div className="space-y-8">
      ...
      <div className="glass-card p-6 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900 flex items-center gap-2">
            <Briefcase className="w-6 h-6 text-blue-600" /> Virtual Portfolio Management
          </h1>
          <p className="text-xs text-slate-600 mt-1">Real-time valuation, unrealized PnL, and sector diversification analytics</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <Card className="space-y-1">
          <span className="text-[11px] font-semibold text-slate-500 uppercase">Available USD Cash</span>
        ...
        <Card className="space-y-1">
          <span className="text-[11px] font-semibold text-slate-500 uppercase">Available INR Cash</span>
          <span className="text-xl font-bold font-mono text-emerald-600 block">
        ...
        <Card className="space-y-1">
          <span className="text-[11px] font-semibold text-slate-500 uppercase">Total Holdings Value</span>
        ...
        <Card className="space-y-1">
          <span className="text-[11px] font-semibold text-slate-500 uppercase">Unrealized PnL (USD)</span>
          <span className={`text-xl font-bold font-mono block ${isPnlPositive ? 'text-emerald-600' : 'text-rose-600'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          <h2 className="text-lg font-bold text-slate-900">Current Stock Holdings</h2>
                  ...
                  portfolio.holdings.map((h) => {
                    ...
                    return (
                      <tr key={h.id} className="hover:bg-blue-50/50 transition-colors">
                        <td className="p-3.5 font-bold text-slate-900">{h.ticker}</td>
                        <td className="p-3.5">
                        ...
                        <td className="p-3.5 font-mono text-slate-900">{formatCurrency(h.current_value, h.market)}</td>
                        <td className={`p-3.5 font-mono font-bold ${isPos ? 'text-emerald-600' : 'text-rose-600'}`}>
        ...
        <div className="space-y-4">
          <h2 className="text-lg font-bold text-slate-900">Sector Breakdown</h2>
          <Card>
      ...
      <div className="space-y-4">
        <h2 className="text-lg font-bold text-slate-900 flex items-center gap-2">
          <History className="w-5 h-5 text-blue-600" /> Recent Execution History
        </h2>
              ...
              {transactions.length === 0 ? (
                ...
                transactions.map((tx) => (
                  <tr key={tx.id} className="hover:bg-blue-50/50 transition-colors">
                    <td className="p-3.5 text-slate-500">{new Date(tx.timestamp).toLocaleString()}</td>
                    <td className="p-3.5">
                      <Badge variant={tx.transaction_type === 'BUY' ? 'green' : 'red'}>{tx.transaction_type}</Badge>
                    </td>
                    <td className="p-3.5 font-bold text-slate-900">{tx.ticker}</td>
                    <td className="p-3.5 font-mono">{tx.quantity}</td>
                    <td className="p-3.5 font-mono">{formatCurrency(tx.price_per_share, tx.currency)}</td>
                    <td className="p-3.5 font-mono font-bold text-slate-900">{formatCurrency(tx.total_amount, tx.currency)}</td>
                  </tr>
                ))
              )}
            </tbody>
