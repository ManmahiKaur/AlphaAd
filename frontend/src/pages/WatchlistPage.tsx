  ...
  return (
    <div className="space-y-8">
      ...
      <div className="glass-card p-6 rounded-2xl flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900 flex items-center gap-2">
            <Bookmark className="w-6 h-6 text-blue-600" /> Watchlist & Price Alerts
          </h1>
          <p className="text-xs text-slate-500 mt-1">Track target price alerts and notes for your favorite stocks</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <Card className="text-center py-12 text-slate-500 text-sm space-y-3">
          ...
          <Link to="/explore" className="inline-block px-4 py-2 bg-blue-600 text-white font-bold rounded-lg text-xs">
            Browse Stocks
          </Link>
        </Card>
      ) : (
            ...
            <Card key={item.id} className="flex flex-col justify-between space-y-4">
              <div>
                <div className="flex items-center justify-between gap-2 mb-1">
                  <span className="font-extrabold text-lg text-slate-900">{item.ticker}</span>
                  <Badge variant={item.market === 'IN' ? 'orange' : 'blue' as any}>{item.market}</Badge>
                </div>
                <h4 className="text-xs text-slate-500 font-medium mb-3">{item.company_name}</h4>
                ...
                {item.target_alert_price && (
                  <div className="text-xs text-amber-600 flex items-center gap-1.5 bg-amber-50 p-2 rounded-lg border border-amber-200">
