    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 glass-card p-6 rounded-2xl">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900 flex items-center gap-2">
            Welcome, {user?.full_name} <span className="text-blue-600">👋</span>
          </h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <Card className="flex items-center gap-4">
          ...
          <div>
            <span className="block text-[11px] font-semibold text-slate-500 uppercase">Virtual Cash ({currency})</span>
        ...
        <Card className="flex items-center gap-4">
          ...
          <div>
            <span className="block text-[11px] font-semibold text-slate-500 uppercase">Total Portfolio Value</span>
            <span className="text-xl font-bold font-mono text-slate-900">
        ...
        <Card className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-xl bg-violet-50 border border-violet-200 text-violet-600 flex items-center justify-center shrink-0">
            <TrendingUp className="w-6 h-6" />
          </div>
          <div>
            <span className="block text-[11px] font-semibold text-slate-500 uppercase">Total Invested</span>
            <span className="text-xl font-bold font-mono text-slate-900">
        ...
        <Card className="flex items-center gap-4">
          ...
          <div>
            <span className="block text-[11px] font-semibold text-slate-500 uppercase">Unrealized PnL</span>
            <span className={`text-xl font-bold font-mono ${(portfolio?.total_pnl_usd || 0) >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-5">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-bold text-slate-900">Trending {country === 'IN' ? 'NSE' : 'US'} Equities</h2>
            <Link to="/explore" className="text-xs text-blue-600 font-bold hover:underline flex items-center gap-1">
        ...
        <div className="space-y-5">
          <h2 className="text-lg font-bold text-slate-900">Featured AI Spotlight</h2>
            ...
            <Card className="neon-border-green space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-bold text-sm text-slate-900">{topPickRec.ticker} AI Rating</span>
                </div>
              ...
              <p className="text-xs text-slate-700 line-clamp-3 bg-blue-50/50 p-3 rounded-lg border border-blue-100">
                {topPickRec.summary}
              </p>

              <div className="flex justify-between items-center text-xs border-t border-slate-200 pt-3">
                <span className="text-slate-500">Target: <strong className="text-blue-600">{formatCurrency(topPickRec.target_price, currency)}</strong></span>
                <Link
          ...
          <Card className="glass-card p-5 space-y-3">
            <h4 className="text-sm font-bold text-slate-900">Ask AI Financial Advisor</h4>
            <p className="text-xs text-slate-600">Have questions about stock trends or portfolio diversification?</p>
            <Link
              to="/ai-chat"
              className="w-full py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs rounded-xl flex items-center justify-center gap-2 transition-all shadow-lg shadow-blue-500/20"
            >
