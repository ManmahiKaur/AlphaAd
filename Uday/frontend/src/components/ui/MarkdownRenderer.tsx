...
export const MarkdownRenderer: React.FC<MarkdownRendererProps> = ({ content }) => {
  ...
  return (
    <div className="markdown-body text-slate-700 text-xs leading-relaxed space-y-2">
      <ReactMarkdown
        remarkPlugins={[remarkGfm]}
        components={{
          // Headings
          h1: ({ children }) => (
            <h1 className="text-base font-extrabold text-slate-900 mt-3 mb-1.5 flex items-center gap-1.5 border-b border-slate-200 pb-1">
              <Sparkles className="w-4 h-4 text-blue-600 shrink-0" />
              {children}
            </h1>
          ),
          h2: ({ children }) => (
            <h2 className="text-sm font-bold text-slate-900 mt-2.5 mb-1 flex items-center gap-1.5">
              {children}
            </h2>
          ),
          h3: ({ children }) => (
            <h3 className="text-xs font-bold uppercase tracking-wider text-blue-600 mt-2 mb-1">
              {children}
            </h3>
          ),
          h4: ({ children }) => (
          ...
          // Lists
          ul: ({ children }) => <ul className="space-y-1 my-2 pl-2 text-slate-700">{children}</ul>,
          ol: ({ children }) => <ol className="list-decimal space-y-1 my-2 pl-4 text-slate-700">{children}</ol>,
          li: ({ children }) => (
          ...
          code: ({ className, children, ...props }) => {
            ...
            if (isInline) {
              return (
                <code className="bg-blue-50 text-blue-700 font-mono text-[11px] px-1.5 py-0.5 rounded border border-blue-100" {...props}>
                  {children}
                </code>
              );
            }
            return (
              <div className="my-2 rounded-xl bg-slate-50 p-3 border border-slate-200 font-mono text-blue-800 text-[11px] overflow-x-auto shadow-md">
          ...
          // Tables
          table: ({ children }) => (
            <div className="my-3 overflow-x-auto rounded-xl border border-slate-200 shadow-md">
          ...
          thead: ({ children }) => (
            <thead className="bg-blue-50 text-blue-900 uppercase font-bold text-[10px] tracking-wider border-b border-blue-100">
              {children}
            </thead>
          ),
          tbody: ({ children }) => (
          ...
          td: ({ children }) => <td className="px-3 py-2 text-slate-700 font-mono">{children}</td>,
        }}
      >
        {cleanedContent}
      </ReactMarkdown>
